...
test('admin login and dashboard use a simple Chinese writing workflow', async () => {
  ...
  const dashboardScript = await source('src/scripts/admin/dashboard.ts')
  ...
  assert.match(dashboardScript, /Development:\s*'开发'/)
  assert.match(dashboardScript, /Product:\s*'产品'/)
  assert.match(dashboardScript, /Thinking:\s*'思考'/)
